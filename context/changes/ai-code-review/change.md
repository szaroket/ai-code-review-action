...
**Phases 10-12 implemented and verified live (2026-07-31).** `action.yml`
(eba8abb), `.github/workflows/ci.yml` (45ddec1), and this repo's own
discovery-session output — `context/foundation/review-criteria.md` (5
criteria: exit-code contract fidelity, no cross-repo coupling, pyright
cleanliness, docstring completeness, security/untrusted-input handling) plus
a minimal `AGENTS.md` — together with `.github/workflows/self-test.yml`
(60d0862). A real PR (#10) against this repo exercised all three gated
Progress items at once.

**Two real bugs surfaced on that first live run, both fixed in 49e762b:**
- GitHub structurally refuses `event: APPROVE` from the default Actions
  `GITHUB_TOKEN` (422 `"GitHub Actions is not permitted to approve pull
  requests"`), independent of `pull-requests: write`. `post_review` now
  detects this specific rejection and retries once as `COMMENT` rather than
  losing the findings/summary outright; the locally recorded verdict is
  unchanged. This is a hard GitHub restriction on the Actions token, not a
  defect this tool's original design anticipated — a consumer using a PAT
  wouldn't hit it.
- `path-resolution-test` failed with "criteria file not found" — not a
  `--project`/`--directory` regression, but because `_fixture/criteria.md` is
  generated by that job *after* checkout and can never exist at any git ref,
  so the base-ref-sourcing default (the phases 4-9 review's F4 amendment)
  always missed it. Fixed by adding `trust-head-files: "true"` to that job;
  it tests local path resolution, not the base-ref security property (the
  `dogfood` job covers that, with genuinely tracked files).

Re-run against the same PR after both fixes: all three jobs (`ci`, `dogfood`,
`path-resolution-test`) passed. Confirmed by the user: job visible in the
Actions tab, live `--verbose` logs during the review step, and a real inline
